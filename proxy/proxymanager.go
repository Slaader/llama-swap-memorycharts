...
func New(proxyConfig config.Config) *ProxyManager {
	...
	pm := &ProxyManager{
		...
		memoryMonitor:  newMemoryMonitor(nil),
	...
	// Set monitor owner and start sampling.
	pm.memoryMonitor.pm = pm
	pm.memoryMonitor.start(pm.shutdownCtx.Done())
...
func (pm *ProxyManager) setupGinEngine() {
	...
	if err != nil {
		...
		pm.ginEngine.GET("/ui/*filepath", func(c *gin.Context) {
			...
			// SPA route fallback: for paths like /ui/memory, serve index.html.
			// Keep direct file requests strict (e.g. .js, .css, .png) to preserve 404 behavior.
			if !strings.Contains(filepath, ".") {
				if _, err := reactFS.Open(filepath); err != nil {
					filepath = "index.html"
				}
			}
