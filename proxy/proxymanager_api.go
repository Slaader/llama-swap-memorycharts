...
func addApiHandlers(pm *ProxyManager) {
	...
	apiGroup := pm.ginEngine.Group("/api", pm.apiKeyAuth())
	{
		...
		apiGroup.GET("/memory/current", pm.apiGetMemoryCurrent)
		apiGroup.GET("/memory/timeline", pm.apiGetMemoryTimeline)
...
func (pm *ProxyManager) apiGetMemoryCurrent(c *gin.Context) {
	snapshot := pm.memoryMonitor.getCurrent()
	c.JSON(http.StatusOK, snapshot)
}

func (pm *ProxyManager) apiGetMemoryTimeline(c *gin.Context) {
	hours := 24 * 30
	bucketHours := 2
	...
	if raw := strings.TrimSpace(c.Query("bucket_hours")); raw != "" {
		parsed, err := strconv.Atoi(raw)
		if err != nil || parsed <= 0 {
			c.JSON(http.StatusBadRequest, gin.H{"error": "bucket_hours must be a positive integer"})
			return
		}
		bucketHours = parsed
	}

	timeline := pm.memoryMonitor.getTimeline(hours, bucketHours)
