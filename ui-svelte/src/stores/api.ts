...
export const memoryCurrent = writable<MemoryCurrent | null>(null);
export const memoryTimeline = writable<MemoryTimelinePoint[]>([]);
...
export async function fetchMemoryCurrent(): Promise<MemoryCurrent | null> {
  try {
    const response = await fetch("/api/memory/current");
    if (!response.ok) {
      throw new Error(`Failed to fetch memory current: ${response.status}`);
    }
    const data: MemoryCurrent = await response.json();
    memoryCurrent.set(data);
    return data;
  } catch (error) {
    console.error("Failed to fetch memory current:", error);
    return null;
  }
}

export async function fetchMemoryTimeline(hours = 24 * 30, bucketHours = 2): Promise<MemoryTimelinePoint[]> {
  try {
    const response = await fetch(`/api/memory/timeline?hours=${hours}&bucket_hours=${bucketHours}`);
    if (!response.ok) {
      throw new Error(`Failed to fetch memory timeline: ${response.status}`);
    }
    const data: MemoryTimelineResponse = await response.json();
    const timeline = data.timeline || [];
    memoryTimeline.set(timeline);
    return timeline;
  } catch (error) {
    console.error("Failed to fetch memory timeline:", error);
    return [];
  }
}
